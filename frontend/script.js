...
const cy = cytoscape({
    ...
    style: [
        // ... (All node and other styles are the same)
        ...
        { 
            selector: 'edge', 
            style: { 
                'width': 3, 'line-color': '#bdc3c7', 'target-arrow-color': '#bdc3c7', 
                'target-arrow-shape': 'triangle', 'curve-style': 'bezier'
            }
        },
        // Style ONLY for edges that HAVE a label property
        {
            selector: 'edge[label]',
            style: {
                'label': 'data(label)', 'font-size': '10px', 'color': '#34495e', 'text-rotation': 'autorotate',
                'text-background-color': '#ecf0f1', 'text-background-opacity': 1, 'text-background-padding': '3px'
            }
        }
    ],
    elements: [], 
...
// (These are all correct from the previous step)
...
async function updateNodeStatus(newStatus) {
    ...
    try {
        await fetch(`http://127.0.0.1:8000/api/nodes/${selectedNode.id()}/status`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: newStatus }), });
...
async function deleteSelectedNode() {
    ...
    try {
        await fetch(`http://127.0.0.1:8000/api/nodes/${nodeId}`, { method: 'DELETE' });
        await loadGraph();
        selectedNode = null;
        closeModal();
        updatePromptUI();
    } catch (error) { console.error("Error deleting node branch:", error); alert("Could not delete node branch."); }
}
...
// (This is correct from the previous step)
submitButton.addEventListener('click', async () => {
    ...
    try {
        ...
        if (selectedNode) { sourceNodeId = data.user_node_id; }
        cy.add([
            ...
            { group: 'edges', data: { id: `edge-${sourceNodeId}-${data.ai_node.id}`, source: sourceNodeId, target: data.ai_node.id, label: promptText } }
        ]);
