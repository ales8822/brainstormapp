...
def init_db(): # ... same ...
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS edges (source_id TEXT NOT NULL, target_id TEXT NOT NULL, FOREIGN KEY (source_id) REFERENCES nodes (id) ON DELETE CASCADE, FOREIGN KEY (target_id) REFERENCES nodes (id) ON DELETE CASCADE, PRIMARY KEY (source_id, target_id))")
    try:
...
class BrainstormRequest(BaseModel): # ... same ...
    prompt: str
    parent_context: Optional[str] = None
    source_node_id: Optional[str] = None
class StatusUpdateRequest(BaseModel): # ... same ...
    status: str
@app.get("/api/graph")
def get_graph(): # ... same ...
    ...
    for node_row in nodes_res:
        ...
        element = {"group": "nodes", "data": node_data, "classes": node_class}
...
def update_node_status(node_id: str, request: StatusUpdateRequest): # ... same ...
...
def delete_node(node_id: str): # ... same ...
