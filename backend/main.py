...
def init_db():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS edges (source_id TEXT NOT NULL, target_id TEXT NOT NULL, label TEXT, FOREIGN KEY (source_id) REFERENCES nodes (id) ON DELETE CASCADE, FOREIGN KEY (target_id) REFERENCES nodes (id) ON DELETE CASCADE, PRIMARY KEY (source_id, target_id))")
    ...
    """)
    # Migrations (these are safe to run multiple times)
    try: cur.execute("ALTER TABLE nodes ADD COLUMN status TEXT NOT NULL DEFAULT 'Idea'"); con.commit()
    except sqlite3.OperationalError: pass
    try: cur.execute("ALTER TABLE edges ADD COLUMN label TEXT"); con.commit()
    except sqlite3.OperationalError: pass
    con.close()
...
class BrainstormRequest(BaseModel):
    prompt: str; parent_context: Optional[str] = None; source_node_id: Optional[str] = None
class StatusUpdateRequest(BaseModel):
    status: str
class ChatMessage(BaseModel):
    role: str; parts: List[str]
class ChatRequest(BaseModel):
    node_context: str = Field(..., alias="nodeContext"); history: List[ChatMessage]; user_message: str = Field(..., alias="userMessage"); node_id: str = Field(..., alias="nodeId")
# --- API Endpoints ---
@app.get("/api/graph")
def get_graph():
...
# --- NEW: Endpoint to get chat history for a node ---
@app.get("/api/nodes/{node_id}/chat")
def get_chat_history(node_id: str):
    ...
    con.close()
    # Format for Gemini API history (role, parts)
...
async def chat_with_idea(request: ChatRequest):
    try:
        ...
        # --- THE FIX: Convert Pydantic models to simple dicts ---
        # The .model_dump() method is a standard Pydantic way to do this.
        ...
        # Save both user and model messages to the database
        con = sqlite3.connect(DB_FILE); cur = con.cursor()
        cur.execute("INSERT INTO chat_messages (id, node_id, role, content) VALUES (?, ?, ?, ?)", (f"msg-{uuid.uuid4()}", request.node_id, "user", request.user_message))
        cur.execute("INSERT INTO chat_messages (id, node_id, role, content) VALUES (?, ?, ?, ?)", (f"msg-{uuid.uuid4()}", request.node_id, "model", ai_message_text))
        con.commit(); con.close()
